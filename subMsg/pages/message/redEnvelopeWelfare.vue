			...
			<template v-if="list.length">
				<view class="card" v-for="item in list" :key="item.id" @click="goDetail(item)">
					...
					<view class="card_content">
						...
						<view class="card_footer" v-if="item.about_table == 'coupon' || item.about_table == 'redPack'">
...
<script>
	export default {
		...
		methods: {
			...
			goDetail(record) {
				if(record.about_table == 'redPack') {
					uni.navigateTo({
						url: "/subUser/pages/user/packet"
					})
				}
				if(record.about_table == 'coupon') {
					uni.navigateTo({
						url: "/subSansong/pages/sansong/ticketManage"
					})
				}
			}
		}
